...
"""HEVC to H.264 CUDA Converter"""
import os, sys, subprocess, argparse, logging, shutil
...
TEMP_DIR = "/home/gh/python/tmp_video_convert"  # NVMe!
...
def check_gpu():
    try:
        ...
        return True
    except: logger.warning("⚠️  nvidia-smi nicht gefunden"); return False

def is_hevc(video_path):
    try:
        result = subprocess.run(['ffprobe', '-v', 'error', '-select_streams', 'v:0', '-show_entries', 'stream=codec_name',
            '-of', 'default=noprint_wrappers=1:nokey=1', str(video_path)], capture_output=True, text=True, check=True, timeout=5)
...
def find_mp4_videos(base_dir, max_depth=10):
    ...
    if not base_path.exists(): logger.error(f"❌ Verzeichnis nicht gefunden: {base_dir}"); return []
    for video in base_path.rglob("*.mp4"):
        try:
            relative = video.relative_to(base_path)
            if len(relative.parts) <= max_depth: videos.append(video)
...
def convert_video_cuda(input_path, temp_dir):
    ...
    logger.info(f"🚀 CUDA-Start: {basename}")
    try:
        cmd = ['ffmpeg', '-y', '-i', str(input_file), '-vf', 'scale=-2:2160', '-c:v', 'h264_nvenc',
            '-preset', 'fast', '-pix_fmt', 'yuv420p', '-movflags', '+faststart', '-c:a', 'copy', str(temp_output)]
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)
        if result.returncode != 0:
            logger.error(f"❌ CUDA-Fehler: {basename}")
            for line in result.stderr.strip().split('\n')[-8:]:
                if line.strip(): logger.error(f"  ffmpeg: {line}")
            return False
        probe = subprocess.run(['ffprobe', '-v', 'error', str(temp_output)], capture_output=True, timeout=10)
        if probe.returncode != 0:
            logger.error(f"❌ Validierung fehlgeschlagen: {basename}")
            temp_output.unlink(missing_ok=True)
            return False
        old_size = input_file.stat().st_size / (1024 * 1024)
        new_size = temp_output.stat().st_size / (1024 * 1024)
        shutil.move(str(temp_output), str(input_file))  # FIX: shutil.move statt replace!
        os.chmod(input_file, 0o664)
        logger.info(f"✅ Erfolg: {basename} ({old_size:.1f}MB → {new_size:.1f}MB)")
        return True
    except Exception as e:
        logger.error(f"❌ Fehler bei {basename}: {e}")
        temp_output.unlink(missing_ok=True)
        return False
...
def main():
    ...
    if not videos: logger.error("❌ Keine MP4-Dateien gefunden"); return 1
    logger.info("Analysiere Codecs...")
    hevc_videos = []
    h264_count = 0
    for i, video in enumerate(videos, 1):
        if i % 10 == 0: logger.info(f"DEBUG: Verarbeite Video {i}/{len(videos)}...")
        if is_hevc(video):
            hevc_videos.append(video)
            if len(hevc_videos) <= 3: logger.info(f"DEBUG: HEVC gefunden: {video.name}")
        else: h264_count += 1
    logger.info(f"Gefunden: {len(videos)} MP4-Dateien ({len(hevc_videos)} HEVC, {h264_count} H.264)")
    if not hevc_videos: logger.info("✅ Keine HEVC-Videos zu konvertieren!"); return 0
    if args.dry_run:
        logger.info("DRY-RUN: Konvertierung übersprungen")
        for video in hevc_videos[:10]: logger.info(f"  - {video}")
        if len(hevc_videos) > 10: logger.info(f"  ... und {len(hevc_videos) - 10} weitere")
        return 0
    ...
    for i, video in enumerate(hevc_videos, 1):
        ...
        if convert_video_cuda(video, temp_dir): success += 1
        else: failed += 1
...
if __name__ == '__main__':
    try: sys.exit(main())
    except KeyboardInterrupt: logger.info("\n⚠️  Abgebrochen"); sys.exit(130)
    except Exception as e: logger.error(f"❌ Fehler: {e}", exc_info=True); sys.exit(1)
